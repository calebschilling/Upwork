...
def transform_time_to_seconds(report_time): # Convert time to seconds.  00:01:30 --> 90 (hours:minutes:seconds)
    ...
    if match:
        ...
        # if hours > 0: # testing purposes only
        #     pass
...
def transform_timestamp_ten_days(most_recent_timestamp): 
    from datetime import datetime, timedelta 

    # take most_recent_timestamp and subtract 10 days from it
    subtract_ten_days = timedelta(days=10)

    minus_ten_day_timestamp = most_recent_timestamp - subtract_ten_days
    logging.info(f"Most recent timestamp: {most_recent_timestamp}")
    logging.info(f"Most recent timestamp - 10 Days: {minus_ten_day_timestamp}")    
    print(minus_ten_day_timestamp)
    return minus_ten_day_timestamp
...
def transform_report(report_result):
    ...
    headers_to_convert_to_seconds  = ["TIME_TILL_ACCEPT", "QUEUE_TIME", "HANDLE_TIME", "INTERACTION_TIME", "RESPONSE_TIME", "EMAIL_QUEUE_TIME"]
    # make the abover all lowercase
    headers_to_convert_to_seconds = [header.lower() for header in headers_to_convert_to_seconds]
...
CREATE TABLE public.emailDetail (
    ID SERIAL PRIMARY KEY,
    CAMPAIGN VARCHAR,
    STATUS VARCHAR,
    TO_ADDRESS VARCHAR,
    TIMESTAMP TIMESTAMP,
    TIME_TILL_ACCEPT INTEGER,
    QUEUE_TIME INTEGER,
    HANDLE_TIME INTEGER,
    ...
    INITIAL_REPLY_BY_AGENT BOOLEAN,
    FROM_ADDRESS VARCHAR,
    EMAIL_SUBJECT VARCHAR,
    EMAIL_QUEUE_TIME INTEGER
);
...
def load_to_postgres(headers, transformed_data, db_cursor):
    ...
    insert_query = "INSERT INTO public.\"emailDetail\" ({}) VALUES ({})".format(', '.join(['\"' + col + '\"' for col in headers]), ', '.join(['%s']*len(headers)))
...
def delete_previous_four_hours(minus_four_hour_timestamp):
    ...
    try:
        sql_query_delete = "DELETE FROM public.\"emailDetail\" WHERE \"timestamp\" >= '{}'".format(minus_four_hour_timestamp)
...
def run_report(report_folder, report_name): # Run report using query to get most recent timestamp for start / end times
    most_recent_email_and_timestamp = "SELECT timestamp FROM public.\"emailDetail\" WHERE DATE(timestamp) = CURRENT_DATE LIMIT 1;"
    ...
    sql_query_most_recent_timestamp = "SELECT MAX(timestamp) + INTERVAL '1 second' FROM \"emailDetail\";" # pst most_recent
    ...
    if most_recent_timestamp is None: # if table is empty start at midnight and end at now 
        ...
        # send most_recent_timestamp through transform_timestamp_ten_hours
        minus_ten_days_timestamp = transform_timestamp_ten_days(most_recent_timestamp)

        # delete previous four hours of data
        delete_previous_four_hours(minus_ten_days_timestamp)
        
        pst_most_recent_minus_four_hours = transform_most_recent_timestamp_to_pst(minus_ten_days_timestamp)
...
try:
    ...
    logging.error(e)
    nty_notification("Call Detail failed to extract the report")
...
try:
    ...
    logging.error(e)
    nty_notification("Call Detail failed to load to Postgres")
